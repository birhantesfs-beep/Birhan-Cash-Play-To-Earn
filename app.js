...
async function submitSignup(){const v=[fullName.value.trim(),email.value.trim(),phone.value.trim(),password.value,confirmPassword.value];if(v.some(x=>!x))return toast("Please complete all fields.");if(v[3]!==v[4])return toast("Passwords do not match.");if(!terms.checked)return toast("Please accept the Terms & Conditions.");const {data,error}=await supabaseClient.auth.signUp({email:v[1],password:v[3],options:{data:{full_name:v[0],phone:v[2]}}});if(error)return toast(error.message);saveUser(v[0],v[1],v[2]);setVerificationEmail(v[1]);clearOtp();route("verification");startVerificationTimer();toast(data.session?"Account created successfully.":"Check your email to verify your account.")}
async function submitLogin(){const id=loginId.value.trim(),pw=loginPassword.value;if(!id||!pw)return toast("Enter your login details.");const {data,error}=await supabaseClient.auth.signInWithPassword({email:id,password:pw});if(error)return toast(error.message);if(remember.checked)localStorage.setItem("birhan_cash_remember","1");setVerificationEmail(id);clearOtp();route("verification");startVerificationTimer()}
