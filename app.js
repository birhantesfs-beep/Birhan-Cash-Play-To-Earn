...
function submitSignup(){const v=[fullName.value.trim(),email.value.trim(),phone.value.trim(),password.value,confirmPassword.value];if(v.some(x=>!x))return toast("Please complete all fields.");if(v[3]!==v[4])return toast("Passwords do not match.");if(!terms.checked)return toast("Please accept the Terms & Conditions.");saveUser(v[0],v[1],v[2]);setVerificationEmail(v[1]);clearOtp();route("verification");startVerificationTimer()}
function submitLogin(){const id=loginId.value.trim();if(!id||!loginPassword.value)return toast("Enter your login details.");if(remember.checked)localStorage.setItem("birhan_cash_remember","1");setVerificationEmail(id);clearOtp();route("verification");startVerificationTimer()}
